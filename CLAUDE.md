...
### Openbaar vervoer — De Lijn (onderzocht, niet in gebruik)

Wij roepen De Lijn niet aan; de reistijd komt van Transitous, zie hieronder. Het onderzoek naar
hun API (welke producten er zijn, dat er géén routeplanner-API meer bestaat, de GTFS-paden en de
licentie) staat in [ROADMAP.md](./ROADMAP.md) onder "De Lijn-API". Ga daar kijken vóór je opnieuw
naar een endpoint zoekt.
...
De volledige werkwijze (bouwen, data verversen, versienummering, changelog, tags, releases,
branch- en PR-afspraken) staat in de skill **`release`** in `.claude/skills/release/`. Roep die
aan zodra er een versie, een branch, een tag of een dataverversing in het spel is.

Twee regels staan hier omdat ze altijd moeten gelden, ook zonder die skill:
...
- **Vraag eerst of het een MAJOR, MINOR of PATCH wordt, vóór je een branch aanmaakt.** Ook bij
  een kleine vraag die niet in de roadmap staat: het versienummer bepaalt de branchnaam. Niet
  zelf inschatten, de gebruiker beslist dat.
- Kleine stappen, één git commit per afgeronde stap. Voor elke stap moet `npm run build` slagen.
- Geen enkele hardgecodeerde schoolnaam of richting in de code.
