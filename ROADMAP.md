...
| --- | --- | --- | --- |
| 1 | Mobiel | Vier ingrepen die samen de zoeker bruikbaar maken op een telefoon. **Sticky resultatenbalk:** vandaag is er op de zoekpagina geen enkel `sticky`- of `fixed`-element behalve de vergelijkbalk, dus na 25 kaarten zit je zo'n 4000 px van het zoekveld, de straal en de filterknop. **Filterpaneel als overlay:** `FilterPanel` staat in de DOM vóór `<main>` (`src/App.tsx:455`), dus opengeklapt schuift het 1589 px tussen het zoekveld en de resultatenkop, die daardoor van y=300 naar y=1869 zakt. Je ziet het aantal resultaten niet meebewegen terwijl je vinkt, en de knop om weer dicht te klappen zit onder het paneel. Op mobiel dus een overlay met eigen scroll en onderaan een vaste knop "Toon 912 resultaten". **Vergelijktabel als stapel:** de tabel is 668 px breed in een venster van 311 px, waarvan de labelkolom er 115 neemt, dus je ziet één school tegelijk. Vergelijken is naast elkaar kijken; op deze breedte wordt het bladeren. Op mobiel geen tabel maar een stapel per school met dezelfde rijlabels. **Tip inklappen:** op 375x667 begint de eerste resultaatkaart pas op y=268 van 667 px, vooral door de tip over deelgemeenten die daar vier regels beslaat. Die pas tonen zodra iemand in het zoekveld tikt | **Klaar om te bouwen, geen bron nodig.** Aanleiding is de bezoekersverdeling van 07/09/2026: 52 procent telefoon, 6 procent tablet, 42 procent laptop. Gemeten in de preview op 375x812 en 375x667. Tablets vallen hier buiten: het `md:`-omslagpunt ligt op 768 px, precies de portretbreedte van een iPad, en op 768 houdt de desktoplayout stand (filterkolom 223 px, alles leesbaar). De tikdoelen en de kaarthoogte uit diezelfde doorlichting zijn geen opmaakwens maar tekortkomingen en staan apart als #42 en #43 |
| 2 | Lijst en kaart naast elkaar | Op desktop vanaf 1280 px drie kolommen: filters 268 vast, lijst flexibel, kaart 470 vast en sticky. Lijst en kaart delen dezelfde hover-toestand, zodat een speld en zijn resultaatkaart samen oplichten. De Lijst/Kaart-schakelaar blijft alleen onder 900 px, waar de kaartkolom wegvalt | **Klaar om te bouwen, geen bron nodig.** Beschreven in de designgids bij de visuele identiteit, maar bewust apart gehouden: dit is gedrag, geen opmaak. Vandaag is `weergave` in `src/App.tsx` een strikte keuze tussen lijst en kaart op elke breedte; er komt gedeelde hover-state bij, de schakelaar wordt afhankelijk van de breedte, en de kaart laadt op desktop altijd mee |
...
- **Vergelijkbalk compacter op mobiel.** Met drie adressen is de balk 162 px hoog, met vier
