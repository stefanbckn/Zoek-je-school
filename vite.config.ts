    ...
    build: {
      rollupOptions: {
        ...
        input: {
          ...
          // alleSteden() leest vestigingen.json zelf in (dezelfde gecommitte dataset als
          // genereer-gemeentepaginas.ts), zodat deze lijst nooit kan afwijken van wat er
          // effectief op gemeente/ staat.
          ...Object.fromEntries(
            alleSteden().map((stad) => [
              `gemeente-${stad.slug}`,
